...
import numpy
from cv import *
#testmagic = magic.Magic(magic_file = 'C:\\pija\\magic')
testmagic = magic.Magic(mime=True)
...
class SkinRegion(object):
    ...
    def __init__(self, image, skin_mask, labeled_image, label_number,
        ...
        self.bounding_rectangle_average_pixel_intensity = np.average(
...
def analize(path):
    ...
    try:
        type_ = testmagic.from_file(path)
    except IOError:
        return False
    ...
    type_ = type_.lower()
    alltype = ['media','mpeg','asf','video']
    if any(x in type_ for x in alltype):
    #if "video" in type_:
...
def analize_numpy_array(image):
    ...
    found = list(HOGDetectMultiScale(fromarray(image), storage, win_stride=(8,8),padding=(32,32), scale=1.05, group_threshold=2))
    image_in_ycbcr = cv2.cvtColor(image, cv2.COLOR_BGR2YCR_CB)
    skin_mask = get_skin_mask(image, image_in_ycbcr)
    face_mask = get_face_mask(image)
    ...
    ImgYCenter = y/2
    #print ImgYCenter
    y1,y2,x1,x2 = get_face_coordinate(image)
    #x1 = abs(x1)
    #y1 = abs(y1)
    #x2 = abs(x2)
    #y2 = abs(y2)
    ...
    #print YFaceCenter
    ICenterTres = 30/100
    ICenterTresX = ImgXCenter*ICenterTres
    ICenterTresY = ImgYCenter*ICenterTres
    ICenterTresXmin = ImgXCenter - ICenterTresX
    ICenterTresXmax = ImgXCenter + ICenterTresX
    ICenterTresYmin = ImgYCenter - ICenterTresY
    ICenterTresYmax = ImgYCenter + ICenterTresY
    ...
    percentage_of_face = face_mask.size / image_pixels_count
    #print skin_pixels_count
    percentage_of_skin = skin_pixels_count / image_pixels_count - percentage_of_face
    #print percentage_of_skin
    if percentage_of_skin < MIN_SKIN_PERCENTAGE:
        return False

    if ((XFaceCenter>ICenterTresXmin)and(XFaceCenter<ICenterTresXmax))and((YFaceCenter>ICenterTresYmin)and(YFaceCenter<ICenterTresYmax)):
        return False
    ...
    if len(regions) > 60 and \
        (regions[0].bounding_rectangle_average_pixel_intensity / 255) < 0.25:

        return False
    #if (face_mask.size==0 and not ((regions[0].region_skin_pixels / skin_pixels_count < 0.35) \
    #    and (regions[1].region_skin_pixels / skin_pixels_count < 0.30) \
    #    and (regions[2].region_skin_pixels / skin_pixels_count < 0.30) or (regions[0].region_skin_pixels / skin_pixels_count < 0.45) or percentage_of_skin < 0.3 and \
    #    (regions[0].bounding_rectangle_size / image_pixels_count) < 0.55 or len(regions) > 60 and \
    #    (regions[0].bounding_rectangle_average_pixel_intensity / 255) < 0.25)):
    #    return False
    return True
...
def get_face_mask(image):
    predictor_path = '/home/me/shape.dat'
    ...
    if (len(dets)>=1):
        ...
        crop_img = image[dets[0].top():dets[0].bottom(),dets[0].left():dets[0].right()]
        image = crop_img.astype(np.uint8)
        imagemask = np.ones(image.shape[:2], dtype="uint8") * 0
        #skinmask[dets[0].top():dets[0].bottom(), dets[0].left():dets[0].right()] = imagemask
        return imagemask
    elif (len(dets)==0):
        img = numpy.empty((0,0),numpy.uint32)
        return img
    else:
        img = numpy.empty((0,0),numpy.uint32)
        return img
def get_face_coordinate(image):
    predictor_path = '/home/me/shape.dat'
    detector = dlib.get_frontal_face_detector()
    predictor = dlib.shape_predictor(predictor_path)
    dets = detector(image, 1)
    ...
    if (len(dets)>=1):
        ...
        #print("Detection {}: Left: {} Top: {} Right: {} Bottom: {}".format(k, d.left(), d.top(), d.right(), d.bottom()))
        shape = predictor(image, dets[0])
        #return {'top':dets[0].top(),'bottom':dets[0].bottom(),'left':dets[0].left(),'right':dets[0].right()}
        return (dets[0].top(),dets[0].bottom(),dets[0].left(),dets[0].right())
    else:
        return (0,0,0,0)
